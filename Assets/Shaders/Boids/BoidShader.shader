    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Interpolators vert (MeshData v, uint instanceID : SV_InstanceID)
            {
                ...
                BoidData boid = boidsIn[instanceID];
                float3 boidAxis = normalize(boid.velocity); 
                float axisProjection = v.positionOS.y;
                float axisDistanceX = v.positionOS.x; 
                
                float3 normalAxis = boidAxis.y > 0.8f ? float3(1,0,0) : float3(0,1,0);
                normalAxis = normalize(cross(normalAxis, boidAxis));
                
                //float3 tangentAxis =  normalize(cross(normalAxis, boidAxis));

                float3 vertexPos = axisProjection * boidAxis + (axisDistanceX) * normalAxis;
                ...
                // THE NORMAL HAS TO BE CORRECTED
                //o.normal = TransformObjectToWorldNormal(v.normal);


                o.positionWS = float4((vertexPos * boidScale + boid.position), 1.0f);
